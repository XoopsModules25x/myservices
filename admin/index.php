...
/**
 * Ajoute une période d'un jour (par exemple "De" ou "à")
 */
function OneTimeSelect($libelle,$de,$jour,$indice,$debutfin)
{
	...
	$heures1 = new XoopsFormSelect($libelle, $nom1, intval(substr($de,0,2)));		// Ajout des heures
	for($i=0; $i<=23; $i++) {
  		$heures1->addOption($i, $i);
	}
	$per_tray->addElement($heures1);
	$minutes1 =  new XoopsFormSelect(' : ', $nom2, intval(substr($de,3,2)));		// Ajout des minutes
	for($i=0; $i<=60; $i++) {
...
/**
 * Ajoute une journée complète (avec ses 2 périodes)
 */
function OneDay($jour,$de1,$a1,$de2,$a2,&$sform)
{
	global $jours;
	$day_tray = new XoopsFormElementTray($jours[$jour] ,'');		// Le tray complet de la journée
	// Première période
	$per1_tray = new XoopsFormElementTray('' ,'');		// Le tray d'une période complète
	...
	// Deuxième période
	$per2_tray = new XoopsFormElementTray('' ,'');		// Le tray d'une période complète
...
$limit = myservices_utils::getModuleOption('perpage');	// Nombre maximum d'éléments à afficher dans l'admin
$baseurl = MYSERVICES_URL.'admin/'.basename(__FILE__);	// URL de ce script
...
switch ($op) {
	...
	// ****************************************************************************************************************
		...
		$opRedirect = 'vat';
		// On vérifie que cette TVA n'est pas utilisée par un produit
	...
	// ****************************************************************************************************************
	case 'employes':		// Liste des employés
	// ****************************************************************************************************************
        xoops_cp_header();
		...
		$itemsCount = 0;
		$tblItems = array();
		$itemsCount = $hMsEmployes->getCount();	// Recherche du nombre total d'éléments
		...
		foreach ($tblItems as $item) {
			...
			echo "<tr class='".$class."'>\n";
			echo "<td><a href='".$item->getEmployeeLink()."' target='_blank'>".$item->getEmployeeFullName()."</a></td><td align='center'>".$item->getVar('employes_email')."</td><td align='center'>".$isActive."</td><td align='center'>".$action_edit.' '.$action_delete."</td>\n";
			echo "<tr>\n";
		}
		$class = ($class == 'even') ? 'odd' : 'even';
		echo "<tr class='".$class."'>\n";
	...
	// ****************************************************************************************************************
	case 'saveeditemployes':	// Sauvegarde d'un employé
	// ****************************************************************************************************************
		xoops_cp_header();
		$id = isset($_POST['employes_id']) ? intval($_POST['employes_id']) : 0;
		$opRedirect = 'employes';
		if(!empty($id)) {
			$edit = true;
		...
		for( $i=1; $i <=5; $i++ ) {
			...
			if(isset($_POST[$fieldName]) && intval($_POST[$fieldName]) == 1) {
				$item->setVar($name, '');
			}
		}
	...
	// ****************************************************************************************************************
	case 'deleteemployes':	// Suppression d'un employé
	// ****************************************************************************************************************
        xoops_cp_header();
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if(empty($id)) {
		...
		// On vérifie que les services de cet employé ne sont pas utilisés dans une commande
	...
	// ****************************************************************************************************************
	case 'addemployes':		// Ajout d'un employé
	case 'editemployes':	// Edition d'un employé
	// ****************************************************************************************************************
        xoops_cp_header();
        myservices_adminMenu(2);
		$objet = 'employes';
    ...
    // ****************************************************************************************************************
    case 'categories':	// Liste des catégories
    // ****************************************************************************************************************
        xoops_cp_header();
		...
		echo "<div class='even'><form method='post' name='quickaccess' id='quickaccess' action='$baseurl' >"._AM_MYSERVICES_LIST." $selectCateg<input type='radio' name='op' id='op' value='editcategories' />"._EDIT." <input type='radio' name='op' id='op' value='deletecategories' />"._DELETE." <input type='submit' name='btnquick' id='btnquick' value='"._GO."' /></form></div>";
	...
	case 'editcategories':		// Edition d'une catégorie
	// ****************************************************************************************************************
        xoops_cp_header();
        myservices_adminMenu(4);
		$objet = 'categories';
	...
	// ****************************************************************************************************************
		xoops_cp_header();
		$id = isset($_POST['categories_id']) ? intval($_POST['categories_id']) : 0;
		...
		if(!empty($id)) {
			...
			$item->unsetNew();
			if(intval($_POST['categories_pid']) == intval($_POST['categories_id'])) {
	...
	// ****************************************************************************************************************
	case 'confdeletecategory':	// Suppression effective d'une catégorie (après confirmation de suppression)
	// ****************************************************************************************************************
        xoops_cp_header();
        myservices_adminMenu(4);
		$id = isset($_POST['id']) ? intval($_POST['id']) : 0;
		...
		$opRedirect = 'categories';
		// On vérifie que cette catégorie n'est pas utilisée par des produits
	...
	// ****************************************************************************************************************
	case 'products':	// Gestion des produits
	// ****************************************************************************************************************
        xoops_cp_header();
		...
		echo "<br /><form method='get' action='$baseurl' name='frmaddedit$objet' id='frmaddedit$objet'>"._MYSERVICES_PRODUCT_ID." <input type='text' name='id' id='id' value='' size='4'/> <input type='radio' name='op' id='op' value='edit$objet' />"._MYSERVICES_EDIT." <input type='radio' name='op' id='op' value='delete$objet' />"._MYSERVICES_DELETE." <input type='submit' name='btngo' id='btngo' value='"._GO."' /></form>";
		...
		echo "<table width='100%' cellspacing='1' cellpadding='3' border='0' class='outer'>";
		echo "<tr><th align='center'>"._MYSERVICES_PRODUCT_TITLE."</th><th align='center'>"._MYSERVICES_PRODUCT_CATEGORY."</th><th align='center'>"._MYSERVICES_PRODUCT_VAT."</th><th align='center'>"._MYSERVICES_PRODUCT_PRICE_HT."</th><th align='center'>"._MYSERVICES_ONLINE."</th><th align='center'>"._AM_MYSERVICES_ACTION."</th></tr>";
		foreach ($tblProducts as $item) {
			...
			echo "<td><a href='".$item->getProductLink()."' target='_blank'>".$item->getVar('products_title')."</a></td><td align='left'>".$productCategory."</td><td align='right'>".$productVat."</td><td align='right'>".$myservices_Currency->amountForDisplay($item->getVar('products_price'))."</td><td align='center'>".$online."</td><td align='center'>".$action_edit.'&nbsp;'.$action_duplicate.'&nbsp;'.$action_delete."</td>\n";
			echo "<tr>\n";
		}
		$class = ($class == 'even') ? 'odd' : 'even';
		echo "<tr class='".$class."'>\n";
	...
	// ****************************************************************************************************************
	case 'addproducts':		// Ajout d'un produit
	case 'editproducts':	// Edition d'un produit
	// ****************************************************************************************************************
        xoops_cp_header();
        myservices_adminMenu(5);
        global $xoopsUser;
		...
		// Recherche des employés qui fournissent ce service
		if($edit) {
		...
		// Images *************************************************************
		$sform->insertBreak("<center><b><u>"._AM_MYSERVICES_PHOTOSDESC."</u></b></center>",'foot');
	...
	// ****************************************************************************************************************
	case 'saveeditproducts':	// Sauvegarde des informations d'un produit
	// ****************************************************************************************************************
		xoops_cp_header();
		$id = isset($_POST['products_id']) ? intval($_POST['products_id']) : 0;
		$objet = 'products';
		if($id > 0) {
			$edit = true;
			$item = $hMsProducts->get($id);
		...
		if($res) {
			...
			if(isset($_POST['employes'])) {
				foreach ($_POST['employes'] as $id2) {
					...
					$item2->setVar('employesproducts_employes_id', intval($id2));
	...
	// ****************************************************************************************************************
	case 'copyproducts':	// Copier un produit
	// ****************************************************************************************************************
        xoops_cp_header();
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		$objet = 'products';
	...
	// ****************************************************************************************************************
	case 'deleteproducts':	// Suppression d'un produit
	// ****************************************************************************************************************
        xoops_cp_header();
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if($id == 0) {
		...
		if(count($tblTmp) == 0) {	// Il n'y a pas de commande rattachée à ce produit
			...
			foreach ($tblTmp2 as $item) {
				...
				echo "<tr class='".$class."'>\n";
				echo "<td align='right'>".$item->getVar('orders_id')."</td><td align='center'>".$date."</td><td align='center'>".$item->getVar('orders_lastname').' '.$item->getVar('orders_firstname')."</td><td align='center'>".$myservices_Currency->amountForDisplay($item->getVar('orders_total'))."</td>\n";
				echo "<tr>\n";
			}
			echo '</table>';
        	show_footer();
	...
	// ****************************************************************************************************************
	case 'holiday':	// Gestion des vacances et des jours de travaillés (mais dont la commande a été passée hors site)
	// ****************************************************************************************************************
        xoops_cp_header();
		...
		// Recherche des personnes en congés
		$start = isset($_GET['start']) ? intval($_GET['start']) : 0;
		...
		foreach ($tblItems as $item) {
			...
			echo '<td>'.$employee."</td><td align='center'>".myservices_utils::sqlDateTimeToFrench($item->getVar('calendar_start'))."</td><td align='center'>".myservices_utils::sqlDateTimeToFrench($item->getVar('calendar_end'))."</td><td align='center'>".$item->getStatusLabel()."</td><td align='center'>".$action_edit.'&nbsp;'.$action_delete."</td>\n";
			echo "<tr>\n";
		}
		$class = ($class == 'even') ? 'odd' : 'even';
		echo "<tr class='".$class."'>\n";
		echo "<td colspan='6' align='center'><form method='post' action='$baseurl' name='frmaddd$objet' id='frmaddd$objet'><input type='hidden' name='status' id='status' value='".CALENDAR_STATUS_HOLIDAY."' /><input type='hidden' name='op' id='op' value='add$objet' /><input type='submit' name='btngo' id='btngo' value='"._AM_MYSERVICES_ADD_ITEM."' /></form></td>\n";
		echo "</tr>\n";
		echo '</table>';
		echo "<div align='right'>".$pagenav->renderNav()."</div>";


		// Jours de fermeture du magasin ******************************************************************************
		...
		$class = ($class == 'even') ? 'odd' : 'even';
		echo "<tr class='".$class."'>\n";
		echo "<td colspan='5' align='center'><form method='post' action='$baseurl' name='frmaddd$objet' id='frmaddd$objet'><input type='hidden' name='status' id='status' value='".CALENDAR_STATUS_CLOSED."' /><input type='hidden' name='op' id='op' value='add$objet' /><input type='submit' name='btngo' id='btngo' value='"._AM_MYSERVICES_ADD_ITEM."' /></form></td>\n";
		echo "</tr>\n";
		echo '</table>';
		echo "<div align='right'>".$pagenav->renderNav()."</div>";
		show_footer();
	...
	// ****************************************************************************************************************
	case 'addholiday':		// Ajouter une personne en congés
	case 'editholiday':		// Editer les congés d'une personne
	// ****************************************************************************************************************
        xoops_cp_header();
        myservices_adminMenu(3);
        $object = 'holiday';
		...
		if( $status == CALENDAR_STATUS_HOLIDAY  || $status == CALENDAR_STATUS_WORK) {
			// Recherche de la liste des employés
			$tblEmployes = $hMsEmployes->getItems();
			if(count($tblEmployes) == 0) {
	...
	// ****************************************************************************************************************
	case 'saveeditholiday':	// Sauvegarde des congés / fermetures
	// ****************************************************************************************************************
		xoops_cp_header();
		$id = isset($_POST['calendar_id']) ? intval($_POST['calendar_id']) : 0;
		$opRedirect = 'holiday';
		if(!empty($id)) {
			$edit = true;
			$item = $hMsCalendar->get($id);
		...
		$item->setVars($_POST);
		if(intval($_POST['calendar_status']) == CALENDAR_STATUS_CLOSED) {
	...
	// ****************************************************************************************************************
	case 'deleteholiday':	// Sauvegarde des congés / fermetures
	// ****************************************************************************************************************
        xoops_cp_header();
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if(empty($id)) {
	...
	// ****************************************************************************************************************
	case 'csvholiday':	// Export des congés en CSV
	// ****************************************************************************************************************
		xoops_cp_header();
        myservices_adminMenu(3);
		...
		if($fp) {
			...
			// Ajout du nom de l'employé(e)
			$entete1[] = 'employee';
			fwrite($fp, implode($s, $entete1)."\n");
			array_pop($entete1);	// Pour supprimer la colonne employee qui a été ajoutée manuellement
	...
	// ****************************************************************************************************************
	case 'orders':	// Gestion des commandes
	// ****************************************************************************************************************
        xoops_cp_header();
		...
		$class='';
		echo "<table width='100%' cellspacing='1' cellpadding='3' border='0' class='outer'>";
		$form ="<form method='post' name='frmfilter' id='frmfilter' action='$baseurl'>". _AM_MYSERVICES_LIMIT_TO." <select name='filter3' id='filter3'><option value='0'".$selected[0].">"._MYSERVICES_CMD_STATE1."</option><option value='1'".$selected[1].">"._MYSERVICES_CMD_STATE2."</option><option value='2'".$selected[2].">"._MYSERVICES_CMD_STATE3."</option><option value='3'".$selected[3].">"._MYSERVICES_CMD_STATE4."</option><option value='4'".$selected[4].">"._MYSERVICES_CMD_STATE5."</option><option value='5'".$selected[5].">"._MYSERVICES_CMD_STATE6."</option></select> <input type='hidden' name='op' id='op' value='orders' /><input type='submit' name='btnfilter' id='btnfilter' value='"._AM_MYSERVICES_FILTER."' /></form>";
		...
		foreach ($tblCommands as $item) {
			...
			echo "<tr class='".$class."'>\n";
			echo "<td align='right'>".$id."</td><td align='center'>".$date."</td><td align='center'>".$item->getVar('orders_lastname').' '.$item->getVar('orders_firstname')."</td><td align='center'>".$myservices_Currency->amountForDisplay($item->getVar('orders_total'))."</td><td align='center'>".$action_validate.' '.$action_edit.' '.$action_delete."</td>\n";
			echo "<tr>\n";
		}
		echo '</table>';
		echo "<div align='right'>".$pagenav->renderNav()."</div>";
	...
	// ****************************************************************************************************************
	case 'deleteorders':	// Suppression d'une commande
	// ****************************************************************************************************************
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if(empty($id)) {
			myservices_utils::redirect(_AM_MYSERVICES_ERROR_1, $baseurl, 5);
	...
	// ****************************************************************************************************************
	case 'validateorders':	// Validation d'une commande
	// ****************************************************************************************************************
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if(empty($id)) {
			myservices_utils::redirect(_AM_MYSERVICES_ERROR_1, $baseurl, 5);
	...
	// ****************************************************************************************************************
		...
		// Comment commander hors pérriode ?
	...
	// ****************************************************************************************************************
	case 'timesheet':	// Horaires de travail
	// ****************************************************************************************************************
        xoops_cp_header();
	...
	// ****************************************************************************************************************
	case 'saveedithours':	// Sauvegarde des horaires de travail
	// ****************************************************************************************************************
		xoops_cp_header();
		$id = isset($_POST['prefs_id']) ? intval($_POST['prefs_id']) : 0;
		...
		for($i=1;$i<=7;$i++) {
			for($j=1;$j<=2;$j++) {
				...
				if( isset($_POST[$nom1]) && isset($_POST[$nom2]) ) {
					if( intval($_POST[$nom1]) == 0 && intval($_POST[$nom2]) == 0 ) {
						$valdeb = '00:00:00';
					} else {
						$valdeb = sprintf("%02d:%02d:00",intval($_POST[$nom1]),intval($_POST[$nom2]));
					}
				}

				if( isset($_POST[$nom3]) && isset($_POST[$nom4]) ) {
					if( intval($_POST[$nom3]) == 0 && intval($_POST[$nom4]) == 0 ) {
						$valfin = '00:00:00';
					} else {
						$valfin = sprintf("%02d:%02d:00",intval($_POST[$nom3]),intval($_POST[$nom4]));
					}
				}
	...
	// ****************************************************************************************************************
		...
		$itemsCount = 15;	// Nombre d'éléments à afficher
		// Affiche les x dernières commandes validées
		$start = isset($_GET['start']) ? intval($_GET['start']) : 0;
		$tblCommands = array();
		...
		foreach($tblCommands as $commande) {
			$class = ($class == 'even') ? 'odd' : 'even';
			// Recherche des éléments de la commande (le panier)
			$critePanier = new Criteria('caddy_orders_id', $commande->getVar('orders_id'), '=');
			$tblCaddy = array();
			$tblCaddy = $hMsCaddy->getObjects($critePanier);
			echo "<tr class='".$class."'>\n";
			echo "<td colspan='5'><b>&raquo;</b> ".myservices_utils::sqlDateTimeToFrench($commande->getVar('orders_date')).' - '.$commande->getVar('orders_lastname').' '.$commande->getVar('orders_firstname').' - '.$commande->getVar('orders_address').' '.$commande->getVar('orders_zip').' '.$commande->getVar('orders_town').' - '.$commande->getVar('orders_telephone').' ('.$commande->getVar('orders_email').')'."</td>\n";
			echo "</tr>\n";
