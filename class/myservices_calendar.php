...
class MyservicesMyservices_calendarHandler extends myservices_ORM
{
	 ...
	 */
	function getClosedDayInMonth($month, $year)
	{
		...
		if(count($closedDates) > 0) {
			foreach($closedDates as $closedDay) {
				...
				if(intval($elementsEndDate['mon']) != intval($month)) {	// Si le mois de la date de fin n'est pas égal au mois traité alors
					$end = $daysMonth;	// On prend le dernier jour du mois
				} else {
					$end = intval($elementsEndDate['mday']);	// Sinon on prend le jour spécifié par la date de fin
				}
				for($i = $start; $i <= $end; $i++) {
					$days[] = $i;
				}
	 ...
	 * @param string $hour L'heure à tester
	 * @return boolean Vrai = magasin fermé, False = magasin ouvert
	 */
	function isStoreClosed($year, $month, $day)
	{
		$startingDay = sprintf("%04d-%02d-01", $year, $month);				// Premier jour du mois
	 ...
	 */
	function isEmployeeAvailable($year, $month, $day, $hour, $duration)
	{
